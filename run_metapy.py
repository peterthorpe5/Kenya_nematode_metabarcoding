...
# 6nt_ index ….   Primer_left _   ACTUALSEQ_    right_primer_   Index(6nt)
# Primers are F: CTGCTGCTGGATCATTACCC and R: CGCCAGCACAGCCGTTAG
for i in sorted(seen_set):
    left = i + "_R1_001.fastq.gz"
    right = i + "_R2_001.fastq.gz"
    commmand = " ".join(["~/apps/public_scripts/metapy/metapy.py",
                         "-l", left,
                         "-r", right,
                         "--left_trim", 
                         "26",
                        "--right_trim", 
                         ...
                         "16",
                         "-d",
                         "/home/pthorpe/scratch/kenya_metab/pool1/Globodera_db.fasta"])#,"--cleanup", "NO"])
    print(commmand)
